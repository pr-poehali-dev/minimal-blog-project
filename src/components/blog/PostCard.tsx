  ...
  if (featured) {
    return (
      <Link to={`/post/${post.slug}`} className="group block border border-stone-200 rounded-2xl p-8 hover:border-amber-300 hover:bg-amber-50/30 transition-all duration-300">
        ...
        <p className="text-stone-500 leading-relaxed mb-5 text-base">{post.excerpt}</p>
        <div className="flex items-center justify-between">
          <div className="flex gap-2 flex-wrap">
            {post.tags.map((tag) => (
              <span key={tag} className="text-xs text-stone-400 hover:text-stone-600">#{tag}</span>
            ))}
          </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-stone-400">{post.read_time} мин</span>
          </div>
